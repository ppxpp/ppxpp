...
package me.ppxpp.library.widget;
...
public class ViewDragHelper {
    ...
    private List<ScrollerCompat> mScrollerPool;
    private Map<View, ScrollerCompat> mScrollerMap;
    ...
    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        ...
        //init scroller pool
        mContext = context;
        mScrollerPool = new ArrayList<>();
        mScrollerMap = new HashMap<>();
    }

    private ScrollerCompat acquireScrollerForView(View targetView){
        ScrollerCompat scroller = mScrollerMap.get(targetView);
        if (scroller == null){
            //get from scroller pool
            /*if (mScrollerPool.size() == 0){
                mScrollerPool.add(ScrollerCompat.create(mContext, sInterpolator));
            }
            scroller = mScrollerPool.remove(0);*/
            scroller = ScrollerCompat.create(mContext, sInterpolator);
            //put into scrollerMap
            mScrollerMap.put(targetView, scroller);
        }
        return scroller;
    }

    private void releaseScrollerForView(View targetView){
        /*ScrollerCompat scroller = mScrollerMap.remove(targetView);
        if (scroller != null){
            //put into pool
            mScrollerPool.add(scroller);
        }*/
    }
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        ...
        ScrollerCompat scrollerCompat = acquireScrollerForView(mCapturedView);
        scrollerCompat.fling(mCapturedView.getLeft(), mCapturedView.getTop(),
                (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId),
                (int) VelocityTrackerCompat.getYVelocity(mVelocityTracker, mActivePointerId),
    ...
    public boolean continueSettling(boolean deferCallbacks) {
        Set<View> keys = mScrollerMap.keySet();
        Set<Map.Entry<View, ScrollerCompat>> entrySet = mScrollerMap.entrySet();
        boolean keepGoing = false;
        //Iterator<View> iterator = keys.iterator();
        int size = mScrollerMap.size();
        Log.d("size", "size = " + size + ", thread = " + Thread.currentThread().getName());
        Iterator<Map.Entry<View, ScrollerCompat>> iterator = entrySet.iterator();
        List<View> keyCache = new ArrayList<>();
        while (iterator.hasNext()){

            size = mScrollerMap.size();
            Log.d("size", "size = " + size + ", thread = " + Thread.currentThread().getName());
            Map.Entry<View, ScrollerCompat> entry = iterator.next();
            //View key = iterator.next();
            View key = entry.getKey();
            //ScrollerCompat scrollerCompat = acquireScrollerForView(key);
            ScrollerCompat scrollerCompat = entry.getValue();//acquireScrollerForView(key);
            boolean going = scrollerCompat.computeScrollOffset();
            final int x = scrollerCompat.getCurrX();
            final int y = scrollerCompat.getCurrY();
            final int dx = x - key.getLeft();
            final int dy = y - key.getTop();

            if (dx != 0) {
                key.offsetLeftAndRight(dx);
            }
            if (dy != 0) {
                key.offsetTopAndBottom(dy);
            }

            if (dx != 0 || dy != 0) {
                mCallback.onViewPositionChanged(key, x, y, dx, dy);
            }

            if (going && x == scrollerCompat.getFinalX() && y == scrollerCompat.getFinalY()) {
                // Close enough. The interpolator/scroller might think we're still moving
                // but the user sure doesn't.
                scrollerCompat.abortAnimation();
                //releaseScrollerForView(key);
                keyCache.add(key);
                going = false;
            }
            keepGoing = keepGoing || going;
        }
        if (!keepGoing) {
            if (deferCallbacks) {
        ...
        for (int i = 0; i < keyCache.size(); i++){
            releaseScrollerForView(keyCache.get(i));
        }
        return keepGoing;
        /*if (mDragState == STATE_SETTLING) {
    ...
    void setDragState(int state) {
        ...
        if (mDragState != state) {
            ...
            if(mDragState == STATE_DRAGGING){
                Log.d("setDragState", "thread:" + Thread.currentThread().getName());
                ScrollerCompat scrollerCompat = acquireScrollerForView(mCapturedView);
